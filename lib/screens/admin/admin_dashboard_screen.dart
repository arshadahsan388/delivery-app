...
class _AdminDashboardScreenState extends ConsumerState<AdminDashboardScreen> with TickerProviderStateMixin {
  ...
  Widget _buildOrdersTab(OrdersState ordersState) {
  ...
  Widget _buildCategoriesTab(AdminData adminData) {
    ...
    if (adminData.error != null) {
      return Center(
        child: Text('Error: ${adminData.error}', style: const TextStyle(color: Colors.white)),
      );
    }

    return _buildCategoriesList(adminData.categories);
  }

  Widget _buildShopsTab(AdminData adminData) {
    if (adminData.isLoading) {
      return const Center(child: CircularProgressIndicator(color: AppTheme.primaryColor));
    }
    ...
    return _buildShopsList(adminData.shops);
  }
  ...
  void _addCategory(String value) {
    if (value.trim().isNotEmpty) {
      final category = CategoryModel(
        ...
        name: value.trim(),
        description: 'Category description for ${value.trim()}',
        icon: Icons.category,
        color: Colors.blue,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Category "${value.trim()}" added successfully!'),
            backgroundColor: Colors.green,
  ...
  void _addShop(String value) {
    if (value.trim().isNotEmpty) {
      final shop = ShopModel(
        id: DateTime.now().millisecondsSinceEpoch.toString(),
        name: value.trim(),
        description: 'Shop description for ${value.trim()}',
        address: 'Default Address, Vehari',
        latitude: 30.0444,
        longitude: 72.357,
        phone: '+92300000000',
        createdAt: DateTime.now(),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Shop "${value.trim()}" added successfully!'),
            backgroundColor: Colors.green,
            duration: const Duration(seconds: 2),
          ),
        );
